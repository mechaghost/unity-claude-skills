...
- **Apple** — POST receipt to `verifyReceipt`. Use the production endpoint first; if it returns status `21007`, retry against sandbox. Better: switch to App Store Server API (`/inApps/v1/transactions/{transactionId}`) which is signed JWS and avoids the sandbox/prod dance. Verify `bundle_id`, `product_id`, transaction uniqueness (store `original_transaction_id`).
- **Google** — call Google Play Developer API `purchases.products.get` or `purchases.subscriptionsv2.get` with the `purchaseToken`. Use a service account with the Android Publisher API enabled. Acknowledge within 3 days or Google auto-refunds.
...
- **Apple** — App Store Server Notifications V2: HTTPS endpoint, payload is Apple-signed JWS. Notification types include `REFUND`, `REVOKE`, `CONSUMPTION_REQUEST`, `DID_RENEW`, `EXPIRED`, `GRACE_PERIOD_EXPIRED`.
- **Google** — Real-time Developer Notifications via Pub/Sub topic on a GCP project; subscription notifications + voided-purchases API for consumables.
